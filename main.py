...
try: 
    ...
    printcolor("error missing xml file, -an xml file should be in the same file as this script if it isnt make one", colors.red)
...
class Calculator:
    def __init__(self):
        ...
        for n in root:
            ...
            self.Seeds.append(seed(Gene(genestring[0]),Gene(genestring[1]),Gene(genestring[2]),Gene(genestring[3]),Gene(genestring[4]),Gene(genestring[5])))
    
        printcolor("succesfully loaded seeds", colors.yellow)
        print()
        print("these are your seeds")
        for n in self.Seeds:
            print(n.GetSeedString())

    def DebugSeed(self):
        #need to fix this
        for n in range(len(self.Seeds)):
            print(n,self.Seeds[n].GetSeedString())
    ...
    def GetSeeds(self):
        ...
        for i in range(self.SeedCount):
            ...
            seed_ = ""
            seed_ = input(": ")
            self.Seeds.append(seed(Gene(seed_[0]),Gene(seed_[1]),Gene(seed_[2]),Gene(seed_[3]),Gene(seed_[4]),Gene(seed_[5])));
            print()

        printcolor("succes these are your seeds: ", colors.yellow)
    ...
    def Bruteforce(self, seeds):
        #goes trough every possible combination yes this is vert inveficens and performance heavy
        ...
        print("\r","100%", end="")
        printcolor("DONE!", colors.yellow)

        print("found", len(GoodSeeds),  "Good Combinations")
